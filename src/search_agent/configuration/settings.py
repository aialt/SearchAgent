...
from ..shared.types import Plan
from .models import get_default_client_config, get_default_action_guard_config
...
class ModelClientConfigs(BaseModel):
    ...
    Attributes:
        ...
        orchestrator (Optional[Dict[str, Any]]): Configuration for the orchestrator component. Default: None.
        web_surfer (Optional[Dict[str, Any]]): Configuration for the web surfer component. Default: None.
        coder (Optional[Dict[str, Any]]): Configuration for the coder component. Default: None.
        file_surfer (Optional[Dict[str, Any]]): Configuration for the file surfer component. Default: None.
        action_guard (Optional[Dict[str, Any]]): Configuration for the action guard component. Default: None.
    """

    orchestrator: Optional[Dict[str, Any]] = None
    web_surfer: Optional[Dict[str, Any]] = None
    coder: Optional[Dict[str, Any]] = None
    file_surfer: Optional[Dict[str, Any]] = None
    action_guard: Optional[Dict[str, Any]] = None
    search_agent: Optional[Dict[str, Any]] = None
    media_agent: Optional[Dict[str, Any]] = None

    default_client_config: ClassVar[Optional[Dict[str, Any]]] = None
    default_action_guard_config: ClassVar[Optional[Dict[str, Any]]] = None

    @classmethod
    def get_default_client_config(cls) -> Dict[str, Any]:
        if cls.default_client_config is None:
            cls.default_client_config = get_default_client_config()
        return cls.default_client_config

    @classmethod
    def get_default_action_guard_config(cls) -> Dict[str, Any]:
        if cls.default_action_guard_config is None:
            cls.default_action_guard_config = get_default_action_guard_config()
        return cls.default_action_guard_config


class SearchAgentConfig(BaseModel):
    """
    Configuration options for Search Agent Framework.
    ...
    Attributes:
        ...
        run_without_docker (bool, optional): Legacy option (not used in current search-only architecture). Default: False.
        network_name (str, optional): The name of the network to use for the browser. Default: "my-network".
        use_a2a_mcp (bool, optional): Use Agent-to-Agent MCP architecture instead of GroupChat/Orchestrator. Default: True.
    """
    ...
    use_a2a_mcp: bool = True  # Default to Agent-to-Agent MCP architecture
